...
class MiniMagickTestBarcode < Barby::Barcode
  def initialize(data)
    @data = data
  end
...
class MiniMagickOutputterTest < Barby::TestCase

  before do
    load_outputter('mini_magick')
    @barcode = MiniMagickTestBarcode.new('10110011100011110000')
    @outputter = MiniMagickOutputter.new(@barcode)
  end
